...
if [ -f frontend/src/shared/api/client.ts ]; then
  ...
  # AIAE convergence (P2 step 4, CR-1): the presigned-upload PUT to S3
  # deliberately bypasses shared/api/client.ts. It cannot go through the
  # typed OpenAPI client (there is no OpenAPI operation for an S3-hosted PUT)
  # and routing it through the backend would violate 14-performance.md by
  # heap-buffering large files. This is a carried, decided, permanent
  # exception (audit §6.2), not a defect — see docs/migration-guardrails.md.
  #
  # The exception used to be a blanket "does any raw fetch/axios/XHR exist"
  # check, which only ever proved >=1. That made it as blind to a second,
  # unrelated site appearing as it was blind to the two known ones vanishing.
  # Made mechanical: each of the two known files must show EXACTLY ONE
  # `await fetch(` call site -- 0 means the exemption silently disappeared
  # (someone rerouted it, and this record is stale), 2 means an undetected
  # second call site appeared in the same file. Everywhere else in the tree
  # stays zero-tolerance, unchanged.
  PRESIGNED_UPLOAD_FILES=(
    "frontend/src/features/library/api/useLessonMutations.ts"
    "frontend/src/features/library/api/useMaterialMutations.ts"
  )
  for presigned_file in "${PRESIGNED_UPLOAD_FILES[@]}"; do
    [ -f "${presigned_file}" ] || fail "CR-1 exemption file missing: ${presigned_file}"
    presigned_count=$(grep -c 'await fetch(' "${presigned_file}" || true)
    [ "${presigned_count}" -eq 1 ] || fail "CR-1: ${presigned_file} must contain exactly 1 presigned-upload fetch() call site (found ${presigned_count})"
  done
  ! grep -RInE 'fetch[[:space:]]*\(|from[[:space:]]+["'\'']axios["'\'']|axios\.|new[[:space:]]+XMLHttpRequest' \
      --include='*.ts' --include='*.tsx' frontend/src \
      --exclude='*.test.ts' --exclude='*.test.tsx' \
      --exclude=useLessonMutations.ts --exclude=useMaterialMutations.ts >/dev/null || {
    fail "Frontend must use shared/api/client.ts, not raw fetch/axios/XMLHttpRequest (outside the two CR-1 presigned-upload exemption files)"
  }

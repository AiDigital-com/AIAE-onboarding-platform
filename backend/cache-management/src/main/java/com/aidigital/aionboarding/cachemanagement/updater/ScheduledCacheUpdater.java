...
/**
 * Polls the shared invalidation log by monotonic database event ID and clears node-local cache
 * regions. The cursor advances only after each event is cleared successfully, so a failed clear is
 * retried on the next poll.
 *
 * <p>The cursor is intentionally node-local. Hibernate/Ehcache regions are heap-local and empty
 * after a node restart, so replaying retained events from sequence zero is safe and avoids
 * timestamp/commit visibility races.
 */
@Component
...
public class ScheduledCacheUpdater {
	...
	/**
	 * Processes bounded event pages. Additional work remains for the next fixed delay, preventing
	 * an old event backlog from monopolizing the scheduler.
	 */
	@Scheduled(
	...
	/**
	 * Returns the node-local polling cursor.
	 *
	 * @return the last successfully processed event sequence
	 */

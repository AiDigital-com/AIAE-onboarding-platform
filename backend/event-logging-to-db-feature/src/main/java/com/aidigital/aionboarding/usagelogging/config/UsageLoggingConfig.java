...
public class UsageLoggingConfig {
	...
	/**
	 * Executor dedicated to fire-and-forget usage event persistence. Bound to Micrometer via
	 * {@link ExecutorServiceMetrics} so queue depth, active/completed counts, and rejections
	 * are visible in {@code /actuator/prometheus} (executor label only — no per-event data).
	 *
	 * @param props         bound app.usage-logging.* properties
	 * @param meterRegistry registry to bind executor metrics to
	 * @return bounded async executor for usage logging writes
	 */
	@Bean(name = "usageLoggingExecutor")
	public TaskExecutor usageLoggingExecutor(UsageLoggingProperties props, MeterRegistry meterRegistry) {
		...
		executor.initialize();
		ExecutorServiceMetrics.monitor(meterRegistry, executor.getThreadPoolExecutor(), "usageLoggingExecutor");
		return executor;
	}

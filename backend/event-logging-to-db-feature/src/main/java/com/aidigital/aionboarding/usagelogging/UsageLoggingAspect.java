...
//  9. A ThreadLocal call-depth guard records ONE event per thread's outermost
//     intercepted call. When an already-logged *ServiceImpl method delegates to
...
public class UsageLoggingAspect {
	...
	/**
	 * Entry point for every intercepted service-impl method. Only the outermost call on this
	 * thread records a usage event; nested calls into other {@code *ServiceImpl} beans proceed
	 * without emitting a duplicate event for the same user-visible action (see class header,
	 * invariant 9).
	 *
	...
	public Object recordUsage(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			return depth == 0 ? recordOutermostUsage(joinPoint) : joinPoint.proceed();
		} finally {
			callDepth.set(depth);
		}
	}
	...
	/**
	 * Records success or failure metadata around the outermost service-impl call on this thread.
	 *
	 * @param joinPoint intercepted service method
	 * @return original method result
	 * @throws Throwable original method failure, always rethrown unchanged
	 */
	Object recordOutermostUsage(ProceedingJoinPoint joinPoint) throws Throwable {
